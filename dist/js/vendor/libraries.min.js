 ...
 */
angular.module("angular-carousel",["ngTouch"]),angular.module("angular-carousel").directive("rnCarouselAutoSlide",["$timeout",function($timeout){return{restrict:"A",link:function(scope,element,attrs){var delay=Math.round(1e3*parseFloat(attrs.rnCarouselAutoSlide)),timer=increment=!1,slidesCount=element.children().length;scope.carouselExposedIndex||(scope.carouselExposedIndex=0),stopAutoplay=function(){angular.isDefined(timer)&&$timeout.cancel(timer),timer=void 0},increment=function(){scope.carouselExposedIndex=scope.carouselExposedIndex<slidesCount-1?scope.carouselExposedIndex+1:0},restartTimer=function(){stopAutoplay(),timer=$timeout(increment,delay)},scope.$watch("carouselIndex",function(){restartTimer()}),restartTimer(),attrs.rnCarouselPauseOnHover&&"false"!=attrs.rnCarouselPauseOnHover&&(element.on("mouseenter",stopAutoplay),element.on("mouseleave",restartTimer)),scope.$on("$destroy",function(){stopAutoplay(),element.off("mouseenter",stopAutoplay),element.off("mouseleave",restartTimer)})}}}]),angular.module("angular-carousel").directive("rnCarouselControls",[function(){return{restrict:"A",replace:!0,scope:{items:"=",index:"="},link:function(scope){scope.prev=function(){scope.index>0&&scope.index--},scope.next=function(){scope.index<scope.items.length-1&&scope.index++}},templateUrl:"carousel-controls.html"}}]),angular.module("angular-carousel").run(["$templateCache",function($templateCache){$templateCache.put("carousel-controls.html",'<div class="rn-carousel-controls">\n  <span class="rn-carousel-control rn-carousel-control-prev" ng-click="prev()" ng-if="index > 0"></span>\n  <span class="rn-carousel-control rn-carousel-control-next" ng-click="next()" ng-if="index < items.length - 1"></span>\n</div>')}]),angular.module("angular-carousel").directive("rnCarouselIndicators",[function(){return{restrict:"A",replace:!0,scope:{items:"=",index:"="},templateUrl:"carousel-indicators.html"}}]),angular.module("angular-carousel").run(["$templateCache",function($templateCache){$templateCache.put("carousel-indicators.html",'<div class="rn-carousel-indicator">\n <span ng-repeat="item in items" ng-click="$parent.index=$index" ng-class="{active: $index==$parent.index}"></span>\n</div>')}]),function(){"use strict";angular.module("angular-carousel").directive("rnCarousel",["$swipe","$window","$document","$parse","$compile","$rootScope",function($swipe,$window,$document,$parse,$compile,$rootScope){var carouselId=0,timeConstant=75,moveTreshold=.05,rubberTreshold=3,requestAnimationFrame=$window.requestAnimationFrame||$window.webkitRequestAnimationFrame||$window.mozRequestAnimationFrame;return{restrict:"A",scope:!0,compile:function(tElement,tAttributes){var repeatItem,repeatCollection,firstChildAttributes=tElement.children()[0].attributes,isRepeatBased=!1,isBuffered=!1,isIndexBound=!1;return tElement.addClass("rn-carousel-slides"),tElement.children().addClass("rn-carousel-slide"),["ng-repeat","data-ng-repeat","x-ng-repeat"].every(function(attr){var repeatAttribute=firstChildAttributes[attr];if(angular.isDefined(repeatAttribute)){var exprMatch=repeatAttribute.value.match(/^\s*([\s\S]+?)\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?\s*$/),trackProperty=exprMatch[3];if(repeatItem=exprMatch[1],repeatCollection=exprMatch[2],repeatItem)return angular.isDefined(tAttributes.rnCarouselBuffered)&&(isBuffered=!0,repeatAttribute.value=repeatItem+" in "+repeatCollection+"|carouselSlice:carouselBufferIndex:carouselBufferSize",trackProperty&&(repeatAttribute.value+=" track by "+trackProperty)),isRepeatBased=!0,!1}return!0}),function(scope,iElement,iAttributes){function updateIndicatorArray(){for(var items=[],i=0;slidesCount>i;i++)items[i]=i;scope.carouselIndicatorArray=items}function getCarouselWidth(){var slides=carousel.children();return containerWidth=0===slides.length?carousel[0].getBoundingClientRect().width:slides[0].getBoundingClientRect().width}function updateContainerWidth(){container.css("width","100%");var width=getCarouselWidth();width&&container.css("width",width+"px")}function scroll(x){isNaN(x)&&(x=scope.carouselIndex*containerWidth),offset=x;var move=-Math.round(offset);move+=scope.carouselBufferIndex*containerWidth,carousel[0].style[transformProperty]=is3dAvailable?"translate3d("+move+"px, 0, 0)":"translate("+move+"px, 0)"}function autoScroll(){var elapsed,delta;amplitude&&(elapsed=Date.now()-timestamp,delta=amplitude*Math.exp(-elapsed/timeConstant),delta>rubberTreshold||-rubberTreshold>delta?(scroll(destination-delta),requestAnimationFrame(autoScroll)):goToSlide(destination/containerWidth))}function capIndex(idx){return idx>=slidesCount?slidesCount:0>=idx?0:idx}function updateBufferIndex(){var bufferIndex=0,bufferEdgeSize=(scope.carouselBufferSize-1)/2;isBuffered&&(bufferIndex=scope.carouselIndex<=bufferEdgeSize?0:slidesCount<scope.carouselBufferSize?0:scope.carouselIndex>slidesCount-scope.carouselBufferSize?slidesCount-scope.carouselBufferSize:scope.carouselIndex-bufferEdgeSize),scope.carouselBufferIndex=bufferIndex}function goToSlide(i,animate){return isNaN(i)&&(i=scope.carouselIndex),animate?(offset=i*containerWidth,void swipeEnd(null,null,!0)):(scope.carouselIndex=capIndex(i),updateBufferIndex(),"$apply"!==$rootScope.$$phase&&"$digest"!==$rootScope.$$phase&&(isIndexBound?scope.$apply():scope.$digest()),void scroll())}function getAbsMoveTreshold(){return moveTreshold*containerWidth}function documentMouseUpEvent(event){swipeMoved=!0,swipeEnd({x:event.clientX,y:event.clientY},event)}function capPosition(x){var position=x;return 0===scope.carouselIndex?position=Math.max(-getAbsMoveTreshold(),position):scope.carouselIndex===slidesCount-1&&(position=Math.min((slidesCount-1)*containerWidth+getAbsMoveTreshold(),position)),position}function swipeStart(coords){return $document.bind("mouseup",documentMouseUpEvent),pressed=!0,startX=coords.x,amplitude=0,timestamp=Date.now(),!1}function swipeMove(coords){var x,delta;return pressed&&(x=coords.x,delta=startX-x,(delta>2||-2>delta)&&(swipeMoved=!0,startX=x,requestAnimationFrame(function(){scroll(capPosition(offset+delta))}))),!1}function swipeEnd(coords,event,forceAnimation){if(!event||swipeMoved){$document.unbind("mouseup",documentMouseUpEvent),pressed=!1,swipeMoved=!1,destination=offset;var minMove=getAbsMoveTreshold(),currentOffset=scope.carouselIndex*containerWidth,absMove=currentOffset-destination,slidesMove=-Math[absMove>=0?"ceil":"floor"](absMove/containerWidth),shouldMove=Math.abs(absMove)>minMove;slidesMove+scope.carouselIndex>=slidesCount&&(slidesMove=slidesCount-1-scope.carouselIndex),slidesMove+scope.carouselIndex<0&&(slidesMove=-scope.carouselIndex);var moveOffset=shouldMove?slidesMove:0;return destination=(moveOffset+scope.carouselIndex)*containerWidth,amplitude=destination-offset,timestamp=Date.now(),forceAnimation&&(amplitude=offset-currentOffset),requestAnimationFrame(autoScroll),!1}}function detect3dSupport(){var has3d,el=document.createElement("p"),transforms={webkitTransform:"-webkit-transform",msTransform:"-ms-transform",transform:"transform"};document.body.insertBefore(el,null);for(var t in transforms)void 0!==el.style[t]&&(el.style[t]="translate3d(1px,1px,1px)",has3d=window.getComputedStyle(el).getPropertyValue(transforms[t]));return document.body.removeChild(el),void 0!==has3d&&has3d.length>0&&"none"!==has3d}function onOrientationChange(){updateContainerWidth(),goToSlide()}carouselId++;var containerWidth,transformProperty,pressed,startX,amplitude,destination,timestamp,offset=0,slidesCount=0,swipeMoved=!1,animOnIndexChange=!0,carousel=iElement.wrap("<div id='carousel-"+carouselId+"' class='rn-carousel-container'></div>"),container=carousel.parent();if((angular.isDefined(iAttributes.rnCarouselIndicator)||angular.isDefined(iAttributes.rnCarouselControl))&&(updateIndicatorArray(),scope.$watch("carouselIndex",function(newValue){scope.indicatorIndex=newValue,scope.carouselExposedIndex=newValue}),scope.$watch("indicatorIndex",function(newValue){goToSlide(newValue,!0)})),angular.isDefined(iAttributes.rnCarouselPreventAnimation)&&(animOnIndexChange=!1),scope.$watch("carouselExposedIndex",function(newValue){goToSlide(newValue,!0)}),angular.isDefined(iAttributes.rnCarouselIndicator)){var indicator=$compile("<div id='carousel-"+carouselId+"-indicator' index='indicatorIndex' items='carouselIndicatorArray' rn-carousel-indicators class='rn-carousel-indicator'></div>")(scope);container.append(indicator)}if(angular.isDefined(iAttributes.rnCarouselControl)){var controls=$compile("<div id='carousel-"+carouselId+"-controls' index='indicatorIndex' items='carouselIndicatorArray' rn-carousel-controls class='rn-carousel-controls'></div>")(scope);container.append(controls)}if(scope.carouselBufferIndex=0,scope.carouselBufferSize=5,scope.carouselIndex=0,iAttributes.rnCarouselIndex){var updateParentIndex=function(value){indexModel.assign(scope.$parent,value)},indexModel=$parse(iAttributes.rnCarouselIndex);angular.isFunction(indexModel.assign)?(scope.$watch("carouselIndex",function(newValue){updateParentIndex(newValue)}),scope.carouselIndex=indexModel(scope),scope.$parent.$watch(indexModel,function(newValue){void 0!==newValue&&(newValue>=slidesCount?(newValue=slidesCount-1,updateParentIndex(newValue)):0>newValue&&(newValue=0,updateParentIndex(newValue)),goToSlide(newValue,animOnIndexChange))}),isIndexBound=!0):isNaN(iAttributes.rnCarouselIndex)||(scope.carouselIndex=parseInt(iAttributes.rnCarouselIndex,10))}isRepeatBased?scope.$watchCollection(repeatCollection,function(newValue){slidesCount=0,angular.isArray(newValue)?slidesCount=newValue.length:angular.isObject(newValue)&&(slidesCount=Object.keys(newValue).length),updateIndicatorArray(),containerWidth||updateContainerWidth(),goToSlide(scope.carouselIndex)}):(slidesCount=iElement.children().length,updateIndicatorArray(),updateContainerWidth()),iAttributes.$observe("rnCarouselSwipe",function(newValue){"false"!==newValue&&"off"!==newValue?$swipe.bind(carousel,{start:swipeStart,move:swipeMove,end:swipeEnd,cancel:function(event){swipeEnd({},event)}}):carousel.unbind()}),isIndexBound||goToSlide(scope.carouselIndex),transformProperty="transform",["webkit","Moz","O","ms"].every(function(prefix){var e=prefix+"Transform";return"undefined"!=typeof document.body.style[e]?(transformProperty=e,!1):!0});var is3dAvailable=detect3dSupport(),winEl=angular.element($window);winEl.bind("orientationchange",onOrientationChange),winEl.bind("resize",onOrientationChange),scope.$on("$destroy",function(){$document.unbind("mouseup",documentMouseUpEvent),winEl.unbind("orientationchange",onOrientationChange),winEl.unbind("resize",onOrientationChange)})}}}}])}(),function(){"use strict";angular.module("angular-carousel").filter("carouselSlice",function(){return function(collection,start,size){return angular.isArray(collection)?collection.slice(start,start+size):angular.isObject(collection)?collection:void 0}})}(),function(){"use strict";angular.module("angular-flexslider",[]).directive("flexSlider",["$parse","$timeout",function($parse,$timeout){return{restrict:"AE",scope:!1,replace:!0,transclude:!0,template:'<div class="flexslider-container"></div>',compile:function(element,attr,linker){return function($scope,$element){var addSlide,collectionString,flexsliderDiv,getTrackFromItem,indexString,match,removeSlide,slidesItems,trackBy;return match=attr.slide.match(/^\s*(.+)\s+in\s+(.*?)(?:\s+track\s+by\s+(.+?))?\s*$/),indexString=match[1],collectionString=match[2],trackBy=$parse(angular.isDefined(match[3])?match[3]:""+indexString),flexsliderDiv=null,slidesItems={},getTrackFromItem=function(collectionItem,index){var locals;return locals={},locals[indexString]=collectionItem,locals.$index=index,trackBy($scope,locals)},addSlide=function(collectionItem,index,callback){var childScope,track;if(track=getTrackFromItem(collectionItem,index),null!=slidesItems[track])throw"Duplicates in a repeater are not allowed. Use 'track by' expression to specify unique keys.";return childScope=$scope.$new(),childScope[indexString]=collectionItem,childScope.$index=index,linker(childScope,function(clone){var slideItem;return slideItem={collectionItem:collectionItem,childScope:childScope,element:clone},slidesItems[track]=slideItem,"function"==typeof callback?callback(slideItem):void 0})},removeSlide=function(collectionItem,index){var slideItem,track;return track=getTrackFromItem(collectionItem,index),slideItem=slidesItems[track],null!=slideItem?(delete slidesItems[track],slideItem.childScope.$destroy(),slideItem):void 0},$scope.$watchCollection(collectionString,function(collection){var attrKey,attrVal,c,currentSlidesLength,e,i,idx,n,options,slider,slides,t,toAdd,toRemove,trackCollection,_i,_j,_k,_l,_len,_len1,_len2,_len3;if(null!=collection?collection.length:void 0){if(null!=flexsliderDiv){for(slider=flexsliderDiv.data("flexslider"),currentSlidesLength=Object.keys(slidesItems).length,null==collection&&(collection=[]),trackCollection={},i=_i=0,_len=collection.length;_len>_i;i=++_i)c=collection[i],trackCollection[getTrackFromItem(c,i)]=c;if(toAdd=function(){var _j,_len1,_results;for(_results=[],i=_j=0,_len1=collection.length;_len1>_j;i=++_j)c=collection[i],null==slidesItems[getTrackFromItem(c,i)]&&_results.push({value:c,index:i});return _results}(),toRemove=function(){var _results;_results=[];for(t in slidesItems)i=slidesItems[t],null==trackCollection[t]&&_results.push(i.collectionItem);return _results}(),1===toAdd.length&&0===toRemove.length||0===toAdd.length){for(_j=0,_len1=toRemove.length;_len1>_j;_j++)e=toRemove[_j],e=removeSlide(e,collection.indexOf(e)),slider.removeSlide(e.element);for(_k=0,_len2=toAdd.length;_len2>_k;_k++)e=toAdd[_k],idx=e.index,addSlide(e.value,idx,function(item){return idx===currentSlidesLength&&(idx=void 0),$scope.$evalAsync(function(){return slider.addSlide(item.element,idx)})});return}}for(slidesItems={},null!=flexsliderDiv&&flexsliderDiv.remove(),slides=angular.element('<ul class="slides"></ul>'),flexsliderDiv=angular.element('<div class="flexslider"></div>'),flexsliderDiv.append(slides),$element.append(flexsliderDiv),i=_l=0,_len3=collection.length;_len3>_l;i=++_l)c=collection[i],addSlide(c,i,function(item){return slides.append(item.element)});options={};for(attrKey in attr)attrVal=attr[attrKey],0!==attrKey.indexOf("$")&&(options[attrKey]=isNaN(n=parseInt(attrVal))?"false"!==attrVal&&"true"!==attrVal?"start"!==attrKey&&"before"!==attrKey&&"after"!==attrKey&&"end"!==attrKey&&"added"!==attrKey&&"removed"!==attrKey?attrVal:function(attrVal){var f;return f=$parse(attrVal),function(slider){return $scope.$apply(function(){return f($scope,{$slider:slider})})}}(attrVal):"true"===attrVal:n);return $timeout(function(){return flexsliderDiv.flexslider(options)},0)}})}}}}])}.call(this);